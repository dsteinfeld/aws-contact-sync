...
class TestRetryLogicProperties:
    ...
    @given(
        ...
        max_delay=st.integers(min_value=5, max_value=15)
    )
    ...
    def test_retry_logic_with_exponential_backoff(self, max_attempts, base_delay, max_delay):
        ...
        **Validates: Requirements 3.1**
        ...
        # Use patch to completely mock boto3.Session to prevent real AWS calls
        with patch('boto3.Session') as mock_session_class:
            ...
            # Create client with no session parameter to use the mocked boto3.Session
            client = AccountManagementClient(retry_config=retry_config)
            ...
            if max_attempts > 1:
                # Calculate expected minimum delay time (reduced for faster tests)
                expected_min_delay = 0
                for attempt in range(max_attempts - 1):  # -1 because last attempt doesn't have delay
                    delay = min(base_delay * (2 ** attempt), max_delay)
                    expected_min_delay += delay * 0.5  # 50% of expected delay (accounting for jitter and test speed)
